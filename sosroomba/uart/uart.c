...
int8_t uart_startTransceiverTx( uint8_t *msg, uint8_t msg_len, uint8_t flags) {
  ...
  // Send one byte and check if we're already done
...
uart_send_interrupt() {
  ...
  // Check if the we're done sending the data
  if (state[TX].idx == state[TX].msgLen) {
