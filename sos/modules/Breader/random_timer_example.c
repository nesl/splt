...
#define RAND_TID	1
#define RAND_TIMER_INTERVAL 256L
    ...
    case MSG_RFID_RESPONSE:
      {
        ...
        if (buff_to_send != NULL){
          ...
          if (sys_id() ==1){
            sys_post_uart(msg->did, MSG_RFID_RESPONSE, msg_len,(void*)buff_to_send, SOS_MSG_RELEASE | SOS_MSG_RELIABLE, BCAST_ADDRESS);
            //sys_led(LED_RED_TOGGLE);
          } else{
            sys_post_net(DFLT_APP_ID0, msg->type, msg_len, (void*)buff_to_send, SOS_MSG_RELEASE, BCAST_ADDRESS);
          }
        }
	//sys_timer_start(RAND_TID, RAND_TIMER_INTERVAL, TIMER_ONE_SHOT);
	uint8_t time = (uint8_t)ker_rand();
	sys_timer_start(RAND_TID, time, TIMER_ONE_SHOT);
        break;
      }
